...
?>
	...
	<br>
	<form method="post" action="form/keranjang/konfirmasi.php?b=<?php echo $totalbiaya ?>">
		
		<div class="form-group">
			<label>Pemesanan di Cabang</label>
			<select class="form-control" name="id_cabang">
				<?php $qcab = mysqli_query($conn, "SELECT * from cabang");
				while ($dcab = mysqli_fetch_array($qcab)) { ?>
				<option value="<?php echo $dcab['id_cabang'] ?>"><?php echo $dcab['nama_cabang'] ?></option>
				<?php } ?>
			</select>
		</div>
